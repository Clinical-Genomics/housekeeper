...
def test_filter_version_by_name_and_bundle_date_returns_the_correct_version(populated_store):
    """Test getting version by bundle name and date."""

    # GIVEN a store with some version
    version_bundles_query = populated_store.session.query(Version).join(Bundle)
    assert version_bundles_query.count() > 0

    # GIVEN a bundle name and creation date
    bundle_name = version_bundles_query.first().bundle.name
    version_date = version_bundles_query.first().created_at

    # WHEN filtering the version bundles by the given name and date
    filtered_version_bundle = filter_version_by_name_and_bundle_date(
        version_bundles=version_bundles_query,
        bundle_name=bundle_name,
        version_date=version_date,
    ).first()

    # THEN a version should be returned
    assert isinstance(filtered_version_bundle, Version)

    # THEN the version's bundle name should match the given name
    assert filtered_version_bundle.bundle.name == bundle_name

    # THEN the version's creation date should match the given date
    assert filtered_version_bundle.created_at == version_date
...
def test_filter_version_by_name_and_bundle_date_with_nonexistent_data_returns_empty_query(populated_store):
    ...
    filtered_versions_query = filter_version_by_name_and_bundle_date(
        version_bundles=populated_store.session.query(Version),
